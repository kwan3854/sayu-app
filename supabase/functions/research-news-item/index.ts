...
serve(async (req) => {
  ...
  try {
    ...
    const { briefing_id, position, topic } = await req.json() as ResearchRequest
...
만약 관련 전문가 의견을 찾을 수 없다면 빈 배열 [] 반환`
    ...
    const { error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        perplexity_search_id: `${briefing_id}-${position}`
      })
    ...
    console.error('Error researching news item:', error)
    return new Response(
